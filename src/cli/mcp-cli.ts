...
const OPENCLAW_MCP_REGISTRY_SCOPE_NOTE =
  "Note: this command only shows ClawWorks-managed mcp.servers entries and does not include mcporter servers from config/mcporter.json.";

export function registerMcpCli(program: Command) {
  const mcp = program
    .command("mcp")
    .description("Manage ClawWorks mcp.servers config and channel bridge");

  mcp
    .command("serve")
    .description("Expose ClawWorks channels over MCP stdio")
  ...
  mcp
    .command("list")
    .description("List ClawWorks-managed MCP servers from mcp.servers")
    ...
    .action(async (opts: { json?: boolean }) => {
      ...
      if (names.length === 0) {
        defaultRuntime.log(
          `No ClawWorks-managed MCP servers configured in ${loaded.path}. Add one with ${formatCliCommand('openclaw mcp set <name> \'{"command":"uvx","args":["context7-mcp"]}\'')}.`,
        );
        defaultRuntime.log(OPENCLAW_MCP_REGISTRY_SCOPE_NOTE);
        return;
      }
      defaultRuntime.log(`ClawWorks-managed MCP servers (${loaded.path}):`);
  ...
  mcp
    .command("show")
    .description("Show one ClawWorks-managed MCP server or the full mcp.servers config")
    ...
    .action(async (name: string | undefined, opts: { json?: boolean }) => {
      ...
      if (name) {
        defaultRuntime.log(`ClawWorks-managed MCP server "${name}" (${loaded.path}):`);
      } else {
        defaultRuntime.log(`ClawWorks-managed MCP servers (${loaded.path}):`);
      }
      printJson(value ?? {});
  ...
  mcp
    .command("set")
    .description("Set one ClawWorks-managed MCP server from a JSON object")
    .argument("<name>", "MCP server name")
  ...
  mcp
    .command("unset")
    .description("Remove one ClawWorks-managed MCP server")
    .argument("<name>", "MCP server name")
    .action(async (name: string) => {
      const loaded = await listConfiguredMcpServers();
      if (!loaded.ok) {
