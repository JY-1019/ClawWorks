/**
 * Shared realtime voice controls for active ClawWorks agent runs.
 *
...
export function buildRealtimeVoiceAgentControlSpeechMessage(text: string): string {
  return [
    "Internal ClawWorks voice control result.",
    "Do not call openclaw_agent_consult or any other tool for this message.",
    "Speak this exact ClawWorks status to the voice call, without adding, removing, or rephrasing words.",
...
export function formatRealtimeVoiceAgentQueueRejection(
  ...
  if (reason === "compacting") {
    return "ClawWorks is compacting the active run and cannot accept voice steering yet.";
  }
  if (reason === "not_streaming") {
    return "ClawWorks has an active run, but it is not currently accepting steering.";
  }
  return mode === "followup"
    ? "ClawWorks could not queue that follow-up."
    : "ClawWorks could not steer the active run.";
}
...
export function formatRealtimeVoiceAgentStatus(params: {
  ...
  if (!params.active) {
    ...
    return turnEnded
      ? "ClawWorks finished the last voice request."
  ...
  if (toolEvent) {
    ...
    if (toolEvent.type === "tool.call") {
      return name ? `ClawWorks is starting ${name}.` : "ClawWorks is starting a tool.";
    }
    if (toolEvent.type === "tool.result") {
      return name
        ? `ClawWorks finished ${name} and is continuing.`
        : "ClawWorks finished a tool and is continuing.";
    }
    if (toolEvent.type === "tool.progress") {
      return name
        ? `ClawWorks is working in ${name}${phase ? ` (${phase})` : ""}.`
        : "ClawWorks is still working.";
    }
  }

  if (params.activity?.activeToolName) {
    return `ClawWorks is running ${params.activity.activeToolName}.`;
  }
  if (params.activity?.activeWorkKind === "model_call") {
    return "ClawWorks is waiting on the model.";
  }
  if (params.activity?.activeWorkKind === "embedded_run" || params.activity?.hasActiveEmbeddedRun) {
    return "ClawWorks is working on the current voice request.";
  }
