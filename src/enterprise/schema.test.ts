...
describe("outward and graph effects", () => {
  const base = (effects: unknown[]) => ({
    schema: "clawworks.workflow-tree",
    schemaVersion: 1,
    id: "acme.outward",
    version: "1.0.0",
    ...
    root: {
      ...
      title: "Root",
      ontology: {
        entities: [{ id: "case", properties: [{ id: "case-id", type: "id", primaryKey: true }] }],
        relationships: [
          { id: "case-covers-case", from: "case", to: "case", cardinality: "one-to-many" },
        ],
        actions: [{ id: "act", parameters: [{ id: "case-id", type: "id" }], effects }],
      },
    },
  });

  it("accepts an action that only calls out", () => {
    expect(validateWorkflowTreeDefinition(base([{ kind: "call", tool: "bank__freeze" }])).ok).toBe(
      true,
    );
  });

  it("accepts a graph effect naming a declared relationship", () => {
    expect(
      validateWorkflowTreeDefinition(base([{ kind: "link", relationship: "case-covers-case" }])).ok,
    ).toBe(true);
  });

  it("rejects a graph effect naming a relationship nothing declares", () => {
    const result = validateWorkflowTreeDefinition(base([{ kind: "link", relationship: "ghost" }]));
    expect(result.ok).toBe(false);
    expect(!result.ok && JSON.stringify(result.issues)).toContain("undeclared relationship");
  });

  it("rejects an action that mixes an outward call with a local write", () => {
    // The call cannot be rolled back into the write transaction, so the two
    // halves can never be one atomic action.
    const result = validateWorkflowTreeDefinition(
      base([
        { kind: "call", tool: "bank__freeze" },
        { entity: "case", kind: "update" },
      ]),
    );
    expect(result.ok).toBe(false);
    expect(!result.ok && JSON.stringify(result.issues)).toContain("mixes an outward call");
  });
});
