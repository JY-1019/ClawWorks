...
import { getEnterpriseActiveRun, resolveEnterpriseMode } from "../../enterprise/runtime.js";
import {
...
import { resolveSessionOwnerAgentIdForKey, resolveSessionStoreKey } from "../session-store-key.js";
...
/** Strict base64: the decoder ignores stray characters, so validate first. */
...
/**
 * A document name must be a plain file name. Path separators, traversal
 * segments, and control characters are rejected at this boundary so no adapter
 * has to defend itself, and a name is never long enough to trip a store's own
 * filesystem limits.
 */
function isSafeDocumentName(name: string): boolean {
  ...
  return !DOCUMENT_NAME_FORBIDDEN.test(name);
}
...
function mapOntology(ontology: PlanNodeRecord["ontology"]): EnterprisePlanNode["ontology"] {
  return {
    ...
    ...(ontology.guidance !== undefined ? { guidance: ontology.guidance } : {}),
    ...(ontology.skills ? { skills: ontology.skills } : {}),
    ...(ontology.mcpServers ? { mcpServers: ontology.mcpServers } : {}),
...
function mapRunDetail(
  ...
  // Only a run this process is executing can publish further step events, so
  // say so explicitly rather than letting a client infer it from `running`.
  // Compared on executionId: the registry is keyed by runId, and a reused runId
  // would otherwise make every historical execution of it look live.
  const active = getEnterpriseActiveRun(record.runId);
  return {
    executionId: record.executionId,
    runId: record.runId,
...
function mapTreeOntology(ontology: OntologyBinding | undefined): EnterpriseTreeOntology {
  ...
  if (ontology.guidance !== undefined) {
    projected.guidance = ontology.guidance;
  }
  if (ontology.skills) {
    projected.skills = [...ontology.skills];
  }
  if (ontology.mcpServers) {
    projected.mcpServers = [...ontology.mcpServers];
  }
...
function buildTreeDetail(entry: WorkflowTreeRegistryEntry): EnterpriseTreeDetail {
  ...
  // Projected because it changes how every step in this tree reads: with it, a
  // step's empty Tools list means "none", not "unrestricted". A client showing
  // per-step scopes without it would describe the opposite of what runs.
  if (entry.tree.capabilityGrants) {
    detail.capabilityGrants = entry.tree.capabilityGrants;
  }
  return detail;
}
...
export const enterpriseHandlers: GatewayRequestHandlers = {
  "enterprise.knowledge.foundations.list": ({ params, respond }) => {
    ...
    const result: EnterpriseKnowledgeFoundationsListResult = {
      foundations: listEnterpriseKnowledgeFoundationDescriptors().map(
        ({ foundationId, descriptor, ownerTreeIds }): EnterpriseKnowledgeFoundationSummary => {
          const summary: EnterpriseKnowledgeFoundationSummary = {
            id: foundationId,
            kind: descriptor.kind,
            displayName: descriptor.displayName,
            // Always sent (empty = plugin-global): retrieval resolves a bundle
            // foundation for its owning tree alone, and a client must be able to
            // tell that from a gateway too old to report ownership at all.
            ownerTreeIds,
  ...
  "enterprise.knowledge.documents.upload": async ({ params, respond }) => {
    ...
    // Buffer.from(_, "base64") silently discards characters outside the
    // alphabet, so "Zm9v!!!!" would decode to "foo" and "!!!!" to an empty
    // file. Reject the payload instead of storing something the caller did not
    // send.
    if (!BASE64_PATTERN.test(params.contentBase64)) {
      respond(
        false,
        undefined,
        errorShape(ErrorCodes.INVALID_REQUEST, "contentBase64 is not valid base64"),
      );
      return;
    }
    // The name becomes a filename in whichever store backs the foundation.
    // LightRAG sanitizes it today, but that is the adapter's business: a name
    // carrying separators or control characters must not reach any adapter.
    if (!isSafeDocumentName(params.name)) {
      respond(
        false,
        undefined,
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          "name must be a plain file name without path separators or control characters",
        ),
      );
      return;
    }
  ...
  "enterprise.runs.list": ({ params, respond }) => {
    ...
    const sessionKey =
      params.sessionKey && cfg
        ...
        : params.sessionKey;
    // Under `session.scope: "global"` the canonical key is shared by every agent,
    // so the key alone would return another agent's run. The owner is derived
    // HERE rather than by the caller: it depends on config (configured default
    // agent, main-key alias, legacy `agent:main:main` remap) that a client does
    // not have. An explicit agentId still wins — that is the client's own
    // selection on the literal `global` key, which config cannot know.
    const derivedAgentId =
      !params.agentId && params.sessionKey && cfg && sessionKey === "global"
        ? resolveSessionOwnerAgentIdForKey({ cfg, sessionKey: params.sessionKey })
        : undefined;
    const agentId = params.agentId ?? derivedAgentId;
    const records = listEnterpriseRunRecords({
      ...(params.limit ? { limit: params.limit } : {}),
      ...(sessionKey ? { sessionKey } : {}),
      ...(agentId ? { agentId } : {}),
      ...(params.chatVisibleOnly ? { chatVisibleOnly: true } : {}),
    });
