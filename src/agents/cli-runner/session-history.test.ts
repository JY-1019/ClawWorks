...
function createSessionTranscript(params: {
  ...
  // same JSONL record order used by persisted ClawWorks sessions.
  const sessionFile =
    params.filePath ??
    path.join(
      params.rootDir,
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders ClawWorks transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[ClawWorks reseed history truncated; older turns dropped]");
