...
export const en: TranslationMap = {
  ...
  enterprise: {
    ...
    ontologyEditor: {
      ...
      // text said actions and derived values were import-only, which is no
      // longer true — both are edited on this screen.
      scopeNote:
      ...
      save: "Save",
      actions: "Actions",
      // Renamed from `actionsNote` so the reworded English re-translates: an action
      // can now also relate two objects, or happen outside this workflow entirely.
      actionsSummary:
      ...
      // Renamed from `readOnlyAction` so the reworded English re-translates: the
      // callout now also covers a write effect whose identity parameter is missing.
      // Renamed again so the reworded English re-translates: the callout now also
      // covers a create missing a required property's parameter.
    ...
    // Each capability kind gets its own block in the step inspector, so these
    // name the KIND and the rows inside it name the list. Renamed from the older
    // `tools`/`skills`/`mcp`/`knowledge`/`deniedTools` keys, which carried the
    // definition key inside the label; the inspector now shows that separately.
    capabilities: {
      toolsTitle: "Tools",
      toolsHint:
      ...
      knowledgeHint:
        "Which knowledge foundations this step lists. Each step on the path carries its own list, and a step keeps only what every list above it also names.",
    },
    bindingDetail: {
