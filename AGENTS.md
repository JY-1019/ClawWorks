...
- Route security through `SECURITY.md`: governance-layer reports to this repo's private advisory, inherited-platform reports upstream. Inherited-platform support goes to upstream's tracker/Discord, not here. `.github/CODEOWNERS` still names upstream teams that do not exist on this fork, so it routes nothing; never guess mentions from it.
...
- Naming: **ClawWorks** product/docs display name; `openclaw` stays for CLI/package/path/config/import ids (never rename machine ids). A brand mention stays `OpenClaw` only when it is a machine value: app bundle/binary/artifact names, state and source paths, service/task/device names a command takes literally, CI workflow names, telemetry identifiers (Codex `serviceName`), the Discord realtime wake word, the OpenRouter attribution header, registered bot identities, and the `--version` line, which the macOS app (`apps/macos/Sources/OpenClaw/GatewayEnvironment.swift`), `install.sh`, and the Parallels harnesses parse by prefix — the decorative banner carries the brand instead. Docs must quote those verbatim, so a doc saying OpenClaw next to ClawWorks prose is usually correct, not a miss.
- Renamed and done: brand chrome, docs prose, README, prompt assembly, tool descriptions, runtime-context overlays, the chat prompt builders in `auto-reply/reply/`, the whole Codex bridge, the realtime voice surface, config help, CLI command descriptions, the onboarding wizard, plugin metadata plus the official catalogs and generated plugin docs. Keep those ClawWorks.
- The rebrand stops there on purpose. A blanket sweep of the remaining strings (agent-path status/error/log messages, doctor text, remaining Control UI copy, code comments) was written, reviewed, and reverted: it broke the `X-OpenClaw-Token` redaction pattern so hook credentials would reach logs, the legacy encrypted OAuth sidecar's Keychain service and `Application Support`/`APPDATA` lookup, the `%LOCALAPPDATA%\OpenClaw\deps\portable-git` directory `install.ps1` creates, the `@openclaw/sdk` public `OpenClaw` export, the persisted `[OpenClaw heartbeat poll]` transcript marker, and the `globalThis.OpenClaw` Canvas bridge saved HTML calls. Those OpenClaw mentions are not violations.
- Renaming anything else is per-string, never bulk. Before changing a brand mention, check whether a shell probe, a Swift/Kotlin client, CI, a persisted record, or an external service reads it; if so, leave it or accept both spellings with the reason stated at the code site. `docs/start/lore.md` needs a rewrite, not a rename: its "The Name" section is the etymology of OpenClaw.
- Third-party plugin compatibility is a hard constraint: plugins published for OpenClaw must keep loading unmodified. `openclaw/plugin-sdk/*` import specifiers, `@openclaw/*` package names, `openclaw.plugin.json` and its schema keys, the `openclaw` block in `package.json`, config keys, `OPENCLAW_*` env vars, and `~/.openclaw` state paths never change. Verify with `pnpm plugin-sdk:api:gen --check` plus a before/after diff of the generated baseline ignoring `sourceLine`.
