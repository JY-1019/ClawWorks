...
- Renamed so far: brand chrome, docs prose, README, prompt assembly and tool descriptions, runtime-context overlays, the chat prompt builders in `auto-reply/reply/`, the whole Codex bridge, and the realtime voice surface end to end. When touching those, keep them ClawWorks.
- Rebrand still pending, known and not violations: status, error, and log strings elsewhere on the agent path (`bash-tools*`, `session-tool-result-guard`, `acp-spawn`, `commands-*`), CLI command descriptions, `config/schema.help.ts`, wizard i18n, doctor messages, Control UI copy other than the sidebar/header/login-gate brand, plugin manifest descriptions plus the docs generated from them (`docs/plugins/reference/**`, `plugin-inventory.md`, `reference.md`), code comments outside the files this sweep touched, and `docs/start/lore.md`, whose "The Name" section is the etymology of OpenClaw and needs a rewrite rather than a rename.
