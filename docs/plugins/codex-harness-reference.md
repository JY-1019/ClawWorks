...
By default, ClawWorks starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after ClawWorks builds its inherited environment.                                                                                                                                                                                                                                                                |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active ClawWorks sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows ClawWorks sandbox egress.                                                                                                                                                                                         |
...
ClawWorks treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before ClawWorks builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent. When native Codex
plugins are configured, ClawWorks uses the connected app-server's plugin control
...
authoritative inventory and metadata source, but ClawWorks policy decides whether
...
and refreshes inventory. Only connect ClawWorks to remote app-servers that are
trusted to accept ClawWorks-managed plugin installs and app inventory refreshes.
...
unattended ClawWorks turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, ClawWorks treats the implicit default as guardian
...
When a ClawWorks sandbox is active, the local Codex app-server process still
runs on the Gateway host. ClawWorks therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the ClawWorks sandbox
backend. Shell access is exposed through ClawWorks sandbox-backed dynamic tools
...
`workspace-write` without active ClawWorks sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`openclaw doctor` and fix the reported host namespace policy for the ClawWorks
...
The stable default is fail-closed: active ClawWorks sandboxing disables native
...
try Codex's remote environment support with ClawWorks's sandbox backend. This
...
When the flag is on and the current ClawWorks session is sandboxed, ClawWorks
...
ClawWorks-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so ClawWorks rejects
that combination.
...
1. An explicit ClawWorks Codex auth profile for the agent.
...
Stdio app-server launches inherit ClawWorks's process environment by default.
ClawWorks owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's ClawWorks state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the ClawWorks agent
instead of leaking in from the operator's personal `~/.codex` home.

ClawWorks does not rewrite `HOME` for normal local app-server launches. Codex-run
...
ClawWorks plugins and ClawWorks skill snapshots still flow through ClawWorks's own
...
part of a ClawWorks agent, inventory them explicitly:
...
Most remaining ClawWorks integration tools, such as messaging, media, cron,
...
surface, while explicit ClawWorks or ACP delegation is still available through
...
Dynamic tool budgets are capped at 600000 ms. On timeout, ClawWorks aborts the
...
post-tool idle budget: after ClawWorks returns an `item/tool/call` response, after
...
`turn/completed`, ClawWorks best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when ClawWorks
...
If discovery fails or times out, ClawWorks uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. ClawWorks
...
For ClawWorks workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
ClawWorks Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact ClawWorks skills
...
`BOOTSTRAP.md` when present is forwarded as ClawWorks turn input reference
context.
