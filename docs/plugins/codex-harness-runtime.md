...
read_when:
  ...
  - You are changing plugin behavior across ClawWorks and Codex harness turns
---
...
Codex mode is not ClawWorks with a different model call underneath. Codex owns more of
the native model loop, and ClawWorks adapts its plugin, tool, session, and
diagnostic surfaces around that boundary.

ClawWorks still owns channel routing, session files, visible message delivery,
ClawWorks dynamic tools, approvals, media delivery, and a transcript mirror.
...
explicit ClawWorks compatibility route keeps the normal ClawWorks system prompt even
...
according to the active Codex thread config. ClawWorks starts and resumes native
Codex threads with Codex's built-in personality disabled so workspace
personality files and ClawWorks agent identity stay authoritative. Lightweight
ClawWorks runs still preserve their existing project-doc suppression. ClawWorks
developer instructions cover ClawWorks runtime concerns such as source-channel
delivery, ClawWorks dynamic tools, ACP delegation, adapter context, and the
active agent workspace profile files. ClawWorks skill catalogs and tool-routed
...
When a ClawWorks session is attached to an existing Codex thread, the next turn
...
normally, and ClawWorks posts the final text to the source conversation. Set
...
Codex heartbeat turns also get `heartbeat_respond` in the searchable ClawWorks
...
| Layer                                 | Owner                     | Purpose                                                             |
| ------------------------------------- | ------------------------- | ------------------------------------------------------------------- |
| ClawWorks plugin hooks                | ClawWorks                 | Product/plugin compatibility across ClawWorks and Codex harnesses.  |
| Codex app-server extension middleware | ClawWorks bundled plugins | Per-turn adapter behavior around ClawWorks dynamic tools.           |
| Codex native hooks                    | Codex                     | Low-level Codex lifecycle and native tool policy from Codex config. |

ClawWorks does not use project or global Codex `hooks.json` files to route
ClawWorks plugin behavior. For the supported native tool and permission bridge,
ClawWorks injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
Codex's app-server reviewer and ClawWorks's approval bridge handle real
...
Codex-level controls. They are not exposed as ClawWorks plugin hooks in the v1
contract.

For ClawWorks dynamic tools, ClawWorks executes the tool after Codex asks for the
call, so ClawWorks fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
ClawWorks can mirror selected events, but it cannot rewrite the native Codex
...
to the matching app-server approval. If a ClawWorks `before_tool_call` hook
...
app-server approval request for the same tool use, ClawWorks opens the plugin
approval prompt and maps the decision back to Codex. Codex `PermissionRequest`
events are a separate approval path and can still route through ClawWorks
...
notifications and ClawWorks adapter state, not native Codex hook commands.
ClawWorks's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke ClawWorks plugin hooks.
...
| ClawWorks channel routing and delivery        | Supported                                                                        | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                                                                                                                                                                                                                                                                                                       |
| ClawWorks dynamic tools                       | Supported                                                                        | Codex asks ClawWorks to execute these tools, so ClawWorks stays in the execution path.                                                                                                                                                                                                                                                                                                                                                                                                 |
| Prompt and context plugins                    | Supported                                                                        | ClawWorks projects ClawWorks-specific prompt/context into the Codex turn while leaving Codex-owned base, model, and configured project-doc prompts in the native Codex lane. ClawWorks disables Codex's built-in personality for native threads so agent workspace personality files remain authoritative. Native Codex developer instructions accept only command guidance explicitly scoped to `codex_app_server`; legacy global command hints remain for non-Codex prompt surfaces. |
...
| Dynamic tool hooks                            | Supported                                                                        | `before_tool_call`, `after_tool_call`, and tool-result middleware run around ClawWorks-owned dynamic tools.                                                                                                                                                                                                                                                                                                                                                                            |
...
| Native permission policy                      | Supported through Codex app-server approvals and compatibility native hook relay | Codex app-server approval requests route through ClawWorks after Codex review. The `PermissionRequest` native hook relay is opt-in for native approval modes because Codex emits it before guardian review.                                                                                                                                                                                                                                                                            |
| App-server trajectory capture                 | Supported                                                                        | ClawWorks records the request it sent to app-server and the app-server notifications it receives.                                                                                                                                                                                                                                                                                                                                                                                      |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                      | Future path                                                                               |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but ClawWorks does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. ClawWorks owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms ClawWorks-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | ClawWorks can request native compaction, but does not receive a stable kept/dropped list, token delta, completion summary, or summary payload.   | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | ClawWorks does not let plugins or context engines veto, rewrite, or replace native Codex compaction.                                             | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | ClawWorks can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Native permissions and MCP elicitations

For `PermissionRequest`, ClawWorks only returns explicit allow or deny decisions
...
ClawWorks remembers that exact provider/session/tool input/cwd fingerprint for a
...
Codex MCP tool approval elicitations are routed through ClawWorks's plugin
...
default `messages.queue.mode: "steer"`, ClawWorks batches steer-mode chat
...
case, ClawWorks waits for the active run to finish before starting the prompt.
...
harness, ClawWorks also calls Codex app-server `feedback/upload` for relevant
...
error. The completed diagnostics reply lists the channels, ClawWorks session ids,
Codex thread ids, and local `codex resume <thread-id>` commands for the threads
that were sent.

If you deny or ignore the approval, ClawWorks does not print those Codex ids and
...
to Codex app-server. ClawWorks does not run preflight compaction for Codex turns,
does not replace Codex compaction with context-engine compaction, and does not
fall back to ClawWorks or public OpenAI summarization when native Codex
compaction cannot be started. ClawWorks keeps a transcript mirror for channel
...
ClawWorks returns after starting that native operation. It does not wait for
completion, impose a separate ClawWorks timeout, restart the shared Codex
app-server, or record the operation as a ClawWorks-completed compaction.

When a context engine requests Codex thread-bootstrap projection, ClawWorks
...
reasoning or plan records when the app-server emits them. Today, ClawWorks only
...
ClawWorks is writing a ClawWorks-owned session transcript tool result.

## Media and delivery

ClawWorks continues to own media delivery and media provider selection. Image,
...
through the normal ClawWorks delivery path. Media generation does not require the legacy runtime.
When Codex emits a native image-generation item with a `savedPath`, ClawWorks
