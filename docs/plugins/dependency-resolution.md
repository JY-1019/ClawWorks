---
summary: "How ClawWorks installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged ClawWorks installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

ClawWorks keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the ClawWorks
package directory.
...
- SDK/core imports are peer or supplied ClawWorks imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into ClawWorks-owned package roots

ClawWorks owns only the plugin lifecycle:
...
ClawWorks uses stable per-source roots:
...
project root for a local npm-pack tarball. ClawWorks reads the tarball's npm
...
`node_modules` beside the plugin package. ClawWorks scans the managed project
...
publishable lockfile during install, and ClawWorks's managed npm project root
supports it through the normal npm install path. ClawWorks-owned publishable
...
ClawWorks does not require it for community packages, but npm will respect it
when present.

ClawWorks-owned npm plugin packages can also publish with explicit
...
dependency. ClawWorks does not let npm install a separate registry copy of the
...
resolution/materialization and ClawWorks reasserts plugin-local
...
Local plugins are treated as developer-controlled directories. ClawWorks does not
...
`doctor --fix` can clean legacy ClawWorks-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of ClawWorks.
...
In source checkouts, ClawWorks treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g openclaw`        | Built runtime tree inside the package | ClawWorks package and explicit plugin install/update/doctor flows    |
...
Older ClawWorks versions generated bundled-plugin dependency roots at startup or
