...
Plugin hooks are in-process extension points for ClawWorks plugins. Use them
...
current plugin options; ClawWorks injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when ClawWorks has resolved the child session's native model before launch.
...
- ClawWorks strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, ClawWorks stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when ClawWorks can
...
the conversation target identifier when ClawWorks can derive one from the session
...
forever. A timeout is logged and ClawWorks continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when ClawWorks can derive a
...
Codex native `Stop` hooks are relayed into this hook as ClawWorks
...
reach the next model turn exactly once. ClawWorks drains queued injections before
...
policy runs from ClawWorks config, covers CLI install and update paths, and fails
closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the ClawWorks process where plugin hooks have
...
field remains in the event payload for compatibility, but ClawWorks no longer
...
context when syncing external wake schedulers, and keep ClawWorks as the
