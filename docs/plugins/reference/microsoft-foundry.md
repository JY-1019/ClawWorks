---
summary: "Adds Microsoft Foundry model provider support to ClawWorks."
read_when:
...
Adds Microsoft Foundry model provider support to ClawWorks.
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. ClawWorks refreshes Microsoft Foundry runtime tokens through
...
ClawWorks uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so ClawWorks
...
deployment through onboarding or include model metadata so ClawWorks can verify
