...
At startup, ClawWorks does roughly this:
...
The manifest is the control-plane source of truth. ClawWorks uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, ClawWorks
...
ClawWorks does not cache plugin discovery results or direct manifest registry
...
ClawWorks still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, ClawWorks calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | ClawWorks tries the normal registry/catalog path first                                                         | _(not a plugin hook)_                                                                                                                         |
...
| 24  | `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared ClawWorks refreshers                                                                                         |
...
that still runs on ClawWorks's normal inference loop.

`resolveUsageAuth` decides whether ClawWorks should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of ClawWorks's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as ClawWorks adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while ClawWorks owns the
...
- ClawWorks only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "ClawWorks plugin runtime helpers",
    count: 5,
  },
});
```
...
`catalog.run(...)` returns the same shape ClawWorks writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to ClawWorks's
...
- if both `catalog` and `discovery` are registered, ClawWorks uses `catalog`
...
When ClawWorks needs setup surfaces for a disabled channel plugin, or
...
this flag. Keep the plugin on the default behavior and let ClawWorks load the
...
ClawWorks can also merge **external channel catalogs** (for example, an MPM
...
This is how ClawWorks stays opinionated without becoming hardcoded to one
