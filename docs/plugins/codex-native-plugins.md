---
summary: "Configure migrated native Codex plugins for Codex-mode ClawWorks agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode ClawWorks agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode ClawWorks agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the ClawWorks turn.

ClawWorks does not translate Codex plugins into synthetic `codex_plugin_*`
ClawWorks dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected ClawWorks agent runtime must be the native Codex harness.
...
`codexPlugins` has no effect on ClawWorks runs, normal OpenAI provider runs, ACP
...
`enable` and `disable` write only to ClawWorks config at
...
- Enabled: ClawWorks config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, ClawWorks
...
ClawWorks reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and ClawWorks rebuilds it
...
A plugin app is exposed only when ClawWorks can map it back to the migrated
...
ClawWorks injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

ClawWorks sets app-level `destructive_enabled` from the effective global or
...
Enabled plugin apps are emitted with `open_world_enabled: true`; ClawWorks does
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, ClawWorks returns a deterministic decline.
- When policy is `true`, ClawWorks auto-accepts only safe schemas it can map to
  an approval response, such as a boolean approve field.
- When policy is `"auto"`, ClawWorks exposes destructive plugin actions to
  Codex but turns ownership-proven MCP approval elicitations into ClawWorks
  plugin approvals before returning the Codex approval response.
- When policy is `"always"`, ClawWorks uses the same Codex write/destructive
...
empty or stale cache. ClawWorks schedules an async refresh and excludes plugin
...
Codex thread bindings keep the app config they started with until ClawWorks
