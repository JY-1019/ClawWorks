...
summary: "Specification for making the bundled Codex app-server harness honor ClawWorks context-engine plugins"
read_when:
  ...
  - You are comparing embedded ClawWorks and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same ClawWorks context-engine
lifecycle contract that embedded ClawWorks turns already honor.
...
ClawWorks-level compaction policy as far as the Codex app-server boundary allows.
...
as built-in ClawWorks attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in ClawWorks-controlled code. The external
boundary is the Codex app-server protocol itself: ClawWorks can control what it
...
Built-in ClawWorks attempts call the context-engine lifecycle directly:
...
For Codex harness turns, ClawWorks should preserve this lifecycle:

1. Read the mirrored ClawWorks session transcript.
...
8. Mirror the Codex result back into the ClawWorks transcript.
...
11. Preserve Codex native compaction signals and ClawWorks compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. ClawWorks should
not try to mutate the app-server's internal history except through supported
protocol calls.

ClawWorks's transcript mirror remains the source for ClawWorks features:
...
The context-engine interface returns ClawWorks `AgentMessage[]`, not a Codex
...
- Label it clearly as ClawWorks assembled context.
...
```text
ClawWorks assembled context for this turn:
...
inside ClawWorks and preserves context-engine semantics.
...
we need strict ClawWorks parity, run context-engine assembly before hook
...
1. ClawWorks context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit ClawWorks compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
ClawWorks compaction should prefer the context engine's `compact()` result for
the ClawWorks transcript mirror and plugin state.
...
the ClawWorks session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
ClawWorks session lifecycle paths. Do not add Codex-specific cleanup unless the
context-engine lifecycle currently misses reset/delete events for all harnesses.

### 10. Error handling

Follow built-in ClawWorks semantics:
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole ClawWorks compaction when the context engine is primary.
...
Avoid requiring lossless-claw in ClawWorks core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for ClawWorks/plugin state.
