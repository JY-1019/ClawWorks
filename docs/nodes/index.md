...
- For direct shell/runtime file executions, ClawWorks also best-effort binds one concrete local
...
```bash
openclaw nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from ClawWorks"}'
```
...
ClawWorks can run a **headless node host** (no UI) that connects to the Gateway
