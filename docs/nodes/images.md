...
- When inbound web messages include media, ClawWorks downloads to a temp file and exposes templating variables:
...
- Media understanding (if configured via `tools.media.*` or shared `tools.media.models`) runs before templating and can insert `[Image]`, `[Audio]`, and `[Video]` blocks into `Body`.
  ...
  - If the active primary image model already supports vision natively, ClawWorks skips the `[Image]` summary block and passes the original image to the model instead.
