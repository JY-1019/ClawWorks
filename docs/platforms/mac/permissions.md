...
- Same path: run the app from a fixed location (for ClawWorks, `dist/OpenClaw.app`).
...
Workaround: move files into the ClawWorks workspace (`~/.openclaw/workspace`) if you want to avoid per-folder grants.
