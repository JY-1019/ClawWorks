...
If you explicitly opt `jq` into `safeBins`, ClawWorks still rejects the `env` builtin in safe-bin
...
- If ClawWorks cannot identify exactly one concrete local file for an interpreter/runtime command
...
session, ClawWorks also resumes that session with an internal followup so the agent observes that
the command did not run instead of later repairing a missing result.

### Followup delivery behavior

After an approved async exec finishes, ClawWorks sends a followup `agent` turn to the same session.
...
the originating channel, ClawWorks keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, ClawWorks sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.openclaw.approval` custom event content on the first prompt
  event so ClawWorks-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. ClawWorks first tries a private route on the
...
topics, ClawWorks preserves the topic for the approval prompt and the post-approval follow-up.
