---
summary: "ClawWorks browser control API, CLI reference, and scripting actions"
read_when:
...
that override because ClawWorks does not launch those browser processes.
...
ClawWorks, then restart the gateway. For Docker, also install the Chromium
...
`OPENCLAW_HOME_VOLUME` or a bind mount. ClawWorks auto-detects the persisted
...
- `upload` and `dialog` are **arming** calls; run them before the click/press that triggers the chooser/dialog. If an action opens a modal, the action response includes `blockedByDialog` and `browserState.dialogs.pending`; pass that `dialogId` to respond directly. Dialogs handled outside ClawWorks appear under `browserState.dialogs.recent`.
...
- Download and trace paths are constrained to ClawWorks temp roots: `/tmp/openclaw{,/downloads}` (fallback: `${os.tmpdir()}/openclaw/...`).
- `upload` accepts files from the ClawWorks temp uploads root and
  ClawWorks-managed inbound media. Managed inbound media can be referenced as
...
Stable tab ids and labels survive Chromium raw-target replacement when ClawWorks
...
- `--format aria`: accessibility tree with `axN` refs. When Playwright is available, ClawWorks binds refs with backend DOM ids to the live page so follow-up actions can use them; otherwise treat the output as inspection-only.
...
ClawWorks supports two "snapshot" styles:
