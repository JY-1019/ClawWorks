...
read_when:
  - Looking for an overview of ClawWorks's media capabilities
...
ClawWorks generates images, videos, and music, understands inbound media
...
For async tools, ClawWorks submits the request to the provider, returns a task
...
ClawWorks wakes the agent with the generated media paths so it can tell the
...
ClawWorks sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until ClawWorks has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in ClawWorks until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
