...
ClawWorks can convert outbound replies into audio across **14 speech providers**
...
<Steps>
  ...
  <Step title="Try it in chat">
    `/tts status` shows the current state. `/tts audio Hello from ClawWorks`
...
ClawWorks picks the first configured provider in registry auto-select order.
...
the voice-design prompt. ClawWorks sends that prompt as the TTS `user` message
...
For each provider attempt, ClawWorks merges configs in this order:
...
<AccordionGroup>
  <Accordion title="Google Gemini">
    ...
    inside the Gemini transcript; ClawWorks does not generate these tags.
  </Accordion>
...
Single command `/tts`. On Discord, ClawWorks also registers `/voice` because
...
- **MiniMax**: MP3 (`speech-2.8-hd` model, 32kHz sample rate) for normal audio attachments. For channel-advertised voice-note targets, ClawWorks transcodes the MiniMax MP3 to 48kHz Opus with `ffmpeg` before delivery when the channel advertises transcoding.
- **Xiaomi MiMo**: MP3 by default, or WAV when configured. For channel-advertised voice-note targets, ClawWorks transcodes Xiaomi output to 48kHz Opus with `ffmpeg` before delivery when the channel advertises transcoding.
- **Local CLI**: uses the configured `outputFormat`. Voice-note targets are
  ...
  with `ffmpeg`.
- **Google Gemini**: Gemini API TTS returns raw 24kHz PCM. ClawWorks wraps it as WAV for audio attachments, transcodes it to 48kHz Opus for voice-note targets, and returns PCM directly for Talk/telephony.
...
- **xAI**: MP3 by default; `responseFormat` may be `mp3`, `wav`, `pcm`, `mulaw`, or `alaw`. ClawWorks uses xAI's batch REST TTS endpoint and returns a complete audio attachment; xAI's streaming TTS WebSocket is not used by this provider path. Native Opus voice-note format is not supported by this path.
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  - The bundled transport accepts an `outputFormat`, but not all formats are available from the service.
  - Output format values follow Microsoft Speech output formats (including Ogg/WebM Opus).
  - Telegram `sendVoice` accepts OGG/MP3/M4A; use OpenAI/ElevenLabs if you need
    guaranteed Opus voice messages.
  - If the configured Microsoft output format fails, ClawWorks retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When `messages.tts.auto` is enabled, ClawWorks:
...
- **Google Gemini:** Returns raw 24 kHz PCM. ClawWorks wraps as WAV for attachments, transcodes to 48 kHz Opus for voice-note targets, returns PCM directly for Talk/telephony.
...
- **Microsoft:** Uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`). Telegram `sendVoice` accepts OGG/MP3/M4A; use OpenAI/ElevenLabs if you need guaranteed Opus voice messages. If the configured Microsoft format fails, ClawWorks retries with MP3.
...
<AccordionGroup>
  <Accordion title="Top-level messages.tts.*">
    ...
    <ParamField path="provider" type="string">
      Speech provider id. When unset, ClawWorks uses the first configured provider in registry auto-select order. Legacy `provider: "edge"` is rewritten to `"microsoft"` by `openclaw doctor --fix`.
    </ParamField>
  ...
  <Accordion title="Xiaomi MiMo">
    ...
    <ParamField path="style" type="string">Optional natural-language style instruction sent as the user message; not spoken. For `mimo-v2.5-tts-voicedesign`, this is the voice-design prompt; ClawWorks supplies a default when omitted.</ParamField>
  </Accordion>
</AccordionGroup>
