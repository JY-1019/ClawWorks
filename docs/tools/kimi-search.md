...
ClawWorks supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), ClawWorks reuses that same host for Kimi
...
If you omit `baseUrl`, ClawWorks defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, ClawWorks defaults to `kimi-k2.6`.
...
ClawWorks treats Kimi `web_search` as successful only after Moonshot returns
...
ClawWorks returns a structured `kimi_web_search_ungrounded` error instead of
