...
Run shell commands in the workspace. `exec` is a mutating shell surface: commands can create, edit, or delete files wherever the selected host or sandbox filesystem permits. Disabling ClawWorks filesystem tools such as `write`, `edit`, or `apply_patch` does not make `exec` read-only.
...
- `timeout` applies to foreground, background, `yieldMs`, gateway, sandbox, and node `system.run` execution. If omitted, ClawWorks uses `tools.exec.timeoutSec`; explicit `timeout: 0` disables the exec process timeout for that call.
...
- On non-Windows gateway hosts, bash and zsh exec commands use a startup snapshot. ClawWorks captures sourceable
...
- ClawWorks sets `OPENCLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
- For channel-origin runs, ClawWorks also exposes a narrow sender/chat identity JSON payload in
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  ClawWorks prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
