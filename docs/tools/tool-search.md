---
summary: "Tool Search: compact large ClawWorks tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want ClawWorks agents to use a large tool catalog without adding every tool schema to the prompt
  - You want ClawWorks tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for ClawWorks runs
---

Tool Search is an experimental ClawWorks agent runtime feature. It gives agents one
...
This page documents ClawWorks Tool Search. It is not the Codex-native tool
...
When enabled for ClawWorks runs, the model receives one `tool_search_code` tool
...
The catalog can include ClawWorks tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through ClawWorks.

Codex harness runs do not receive these experimental ClawWorks Tool Search
controls. ClawWorks passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the ClawWorks embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible ClawWorks and plugin tools.
...
6. Expose the ClawWorks code bridge, the structured fallback tools, or the
   compact directory surface to the model.

At execution time every real tool call returns to ClawWorks. The isolated Node
...
- `directory`: exposes `tool_search`, `tool_describe`, and `tool_call` plus a
  ...
  providers that should see tool names without every full schema. ClawWorks can
  also expose a small bounded set of likely or required tool schemas directly
  for the current turn.

All modes use the same policy-filtered catalog and normal ClawWorks execution
...
for the current run while ClawWorks tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small ClawWorks tool
...
catalog includes eligible ClawWorks, MCP, and client tools after normal policy
filtering.
...
Calls a selected tool through ClawWorks.
...
the model requests an exact hidden directory tool name directly, ClawWorks
hydrates it from the authorized catalog before normal execution.
Directory-mode client tool names must not collide with ClawWorks, plugin, or MCP
...
network grants, and no child-process or worker grants. ClawWorks enforces a
...
Normal ClawWorks behavior still applies to final calls:
...
Enable Tool Search for ClawWorks runs with the default code bridge:
...
Use the structured fallback tools instead for ClawWorks runs:
...
Use the compact directory surface instead for ClawWorks runs:
...
- final tool calls executed through ClawWorks
...
- whether the result came from ClawWorks, MCP, or a client tool

## E2E validation

The QA Lab gateway scenario proves both paths with the ClawWorks runtime:
