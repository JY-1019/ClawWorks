---
summary: "Use OpenShell as a managed sandbox backend for ClawWorks agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for ClawWorks. Instead of running Docker
containers locally, ClawWorks delegates sandbox lifecycle to the `openshell` CLI,
...
- ClawWorks Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, ClawWorks creates an OpenShell
...
- You edit files locally outside ClawWorks and want those changes visible in the
...
- When the sandbox is first created, ClawWorks seeds the remote workspace from
...
- ClawWorks does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside ClawWorks after the initial seed, the remote sandbox does **not** see those changes. Use `openclaw sandbox recreate` to re-seed.
</Warning>
...
1. ClawWorks calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. ClawWorks calls `openshell sandbox ssh-config <name>` to get SSH connection
