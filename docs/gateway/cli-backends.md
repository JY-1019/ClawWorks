...
ClawWorks can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **ClawWorks tools are not injected directly**, but backends with `bundleMcp: true`
...
gateway host, ClawWorks now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), ClawWorks will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw ClawWorks transcript history before compaction.
...
2. **Builds a system prompt** using the same ClawWorks prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   ClawWorks session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us ClawWorks-style Claude CLI usage is allowed again, so ClawWorks treats
...
resolver for ClawWorks skills. When the current skills snapshot includes at least
one selected skill with a materialized path, ClawWorks passes a temporary Claude
Code plugin with `--plugin-dir` and omits the duplicate ClawWorks skills catalog
from the appended system prompt. If the snapshot has no materialized plugin
skill, ClawWorks keeps the prompt catalog as a fallback. Skill env/API key
overrides are still applied by ClawWorks to the child process environment for the
run.

Claude CLI also has its own noninteractive permission mode. ClawWorks maps that
to the existing exec policy instead of adding Claude-specific policy config.
For ClawWorks-managed Claude live sessions, the effective ClawWorks exec policy is
...
Claude launches normalize that flag to match the effective ClawWorks exec policy.

The bundled Anthropic `claude-cli` backend also maps ClawWorks `/think` levels
...
Before ClawWorks can use the bundled `claude-cli` backend, Claude Code itself
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, ClawWorks resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; ClawWorks clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from ClawWorks's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. ClawWorks still keeps raw
...
- ClawWorks drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, ClawWorks lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), ClawWorks seeds
...
provider would start cold because ClawWorks's own session transcript is empty
for `claude-cli` runs.
...
ClawWorks will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, ClawWorks appends the
...
- For Gemini CLI JSON output, ClawWorks reads reply text from `response` and usage
...
agent runs use the Codex app-server harness through `openai/*`; ClawWorks no
...
- If you override Gemini args to `--output-format json`, ClawWorks normalizes that
...
rewrites streamed assistant deltas and parsed final text before ClawWorks handles
...
Some CLI backends run an agent that compacts its **own** transcript, so ClawWorks must
...
`ownsNativeCompaction: true`, and ClawWorks returns a no-op from the compaction path.
...
`contextTokens: 1_000_000` purely to keep ClawWorks's safeguard from firing on a
...
CLI backends do **not** receive ClawWorks tool calls directly, but a backend can
...
When bundle MCP is enabled, ClawWorks:
...
If no MCP servers are enabled, ClawWorks still injects a strict config when a
...
When a fresh CLI session is seeded from a prior ClawWorks transcript (for
...
- **No direct ClawWorks tool calls.** ClawWorks does not inject tool calls into
