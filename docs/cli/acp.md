...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to a ClawWorks Gateway.
...
If you want an external MCP client to talk directly to ClawWorks channel
...
- ClawWorks acts as an ACP server
- an IDE or ACP client connects to ClawWorks
- ClawWorks forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where ClawWorks runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to ClawWorks: use `openclaw acp`
- ClawWorks should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include ClawWorks parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                               |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the ClawWorks gateway or agent instead.                                                                                                                                    |
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run ClawWorks through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.
...
it to drive a ClawWorks Gateway session.
...
If you want ACPX-backed sessions to see ClawWorks plugin tools or selected
...
[ClawWorks tools MCP bridge](/tools/acp-agents-setup#openclaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
ClawWorks bot over ACP, use `acpx` with its built-in `openclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx openclaw` at `openclaw acp`.
3. Target the ClawWorks session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default ClawWorks ACP session
acpx openclaw exec "Summarize the active ClawWorks session state."

# Persistent named session for follow-up turns
acpx openclaw sessions ensure --name codex-bridge
acpx openclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my ClawWorks work agent for recent context relevant to this repo."
```
...
For a repo-local ClawWorks checkout, use the direct CLI entrypoint instead of the
...
pull contextual information from a ClawWorks agent without scraping a terminal.
...
In Zed, open the Agent panel and select "ClawWorks ACP" to start a thread.
