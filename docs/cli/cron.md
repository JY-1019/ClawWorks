...
Use `--command` for deterministic shell-style jobs that should run inside ClawWorks cron without starting an isolated agent/model run:
...
- When a job has `--model` but no fallback list is configured, ClawWorks passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
