...
read_when:
  - You want to migrate from Hermes or another agent system into ClawWorks
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local ClawWorks state exists.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Backups">
    Apply creates and verifies a ClawWorks backup before applying the migration. If no local ClawWorks state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the ClawWorks agent workspace.
...
- Claude command Markdown files converted into ClawWorks skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. ClawWorks does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the ClawWorks Codex harness and you want to
...
start checked unless the target ClawWorks Codex plugin config already has that
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  ClawWorks agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to ClawWorks provider runs,
...
- `SOUL.md` and `AGENTS.md` into the ClawWorks agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for ClawWorks file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that ClawWorks cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live ClawWorks config or credentials. This preserves opaque or unsafe state without pretending ClawWorks can execute or trust it automatically:
...
<Note>
Onboarding imports require a fresh ClawWorks setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move ClawWorks to a new machine.
