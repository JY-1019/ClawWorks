...
read_when:
  - You want a first-class backup archive for local ClawWorks state
...
Create a local backup archive for ClawWorks state, config, auth profiles, channel/provider credentials, sessions, and optionally workspaces.
...
- If the current working directory is inside a backed-up source tree, ClawWorks falls back to your home directory for the default archive location.
...
`openclaw backup create` plans backup sources from your local ClawWorks install:

- The state directory returned by ClawWorks's local state resolver, usually `~/.openclaw`
...
If you use `--only-config`, ClawWorks skips state, credentials-directory, and workspace discovery and archives only the active config file path.

ClawWorks canonicalizes paths before building the archive. If config, the
...
ClawWorks does not enforce a built-in maximum backup size or per-file size limit.
...
- Filesystem behavior at the destination path. ClawWorks prefers a no-overwrite hard-link publish step and falls back to exclusive copy when hard links are unsupported
