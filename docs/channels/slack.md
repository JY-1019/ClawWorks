...
| Request signing              | Not used (auth is the App-Level Token)                                                                                                                | Slack signs every request; ClawWorks verifies with `signingSecret`                                             |
| Recovery on connection drop  | Slack SDK auto-reconnect is enabled; ClawWorks also restarts failed Socket Mode sessions with bounded backoff. Pong-timeout transport tuning applies. | No persistent connection to drop; retries are per-request from Slack                                           |
...
Relay mode separates Slack ingress from the ClawWorks gateway. A trusted router owns the
...
```json Recommended
{
  "display_information": {
    "name": "ClawWorks",
    "description": "Slack connector for ClawWorks"
  },
  "features": {
    "bot_user": { "display_name": "ClawWorks", "always_online": true },
    "app_home": {
    ...
    "assistant_view": {
      "assistant_description": "ClawWorks connects Slack assistant threads to ClawWorks agents.",
      "suggested_prompts": [
    ...
    "slash_commands": [
      {
        "command": "/openclaw",
        "description": "Send a message to ClawWorks",
        "should_escape": false
      }
    ]
  },
...
ClawWorks sets the Slack SDK client pong timeout to 15 seconds by default for Socket Mode. Override the transport settings only when you need workspace- or host-specific tuning:
...
- Only `clientPingTimeout` has a ClawWorks default (`15000`). `serverPingTimeout` and `pingPongLoggingEnabled` are passed to the Slack SDK only when configured.
...
The default manifest enables the Slack App Home **Home** tab and subscribes to `app_home_opened`. When a workspace member opens the Home tab, ClawWorks publishes a safe default Home view with `views.publish`; no conversation payload or private configuration is included. The **Messages** tab remains enabled for Slack DMs. The manifest also enables Slack assistant threads with `features.assistant_view`, `assistant:write`, `assistant_thread_started`, and `assistant_thread_context_changed`; assistant threads route to their own ClawWorks thread sessions and keep Slack-provided thread context available to the agent.
...
- ClawWorks seeds an eligible top-level channel root into `agent:<agentId>:slack:channel:<channelId>:thread:<rootTs>` when that root is expected to start a visible Slack thread, so the root and later thread replies share one ClawWorks session. This applies to `app_mention` events, explicit bot or configured mention-pattern matches, and `requireMention: false` channels with non-`off` `replyToMode`.
...
`ackReaction` sends an acknowledgement emoji while ClawWorks is processing an inbound message. `ackReactionScope` decides _when_ that emoji is actually sent.
...
- Top-level Slack DMs stay off-thread by default, so they do not show Slack's thread-style native stream/status preview; ClawWorks posts and edits a draft preview in the DM instead.
...
- If streaming fails mid-reply, ClawWorks falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while ClawWorks is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, ClawWorks keeps processing the message and falls back to the file placeholder.
...
- The interactive callback values are ClawWorks-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, ClawWorks falls back to the original text reply instead of sending an invalid blocks payload.

### Plugin-owned modal submissions

Slack plugins that register an interactive handler can also receive modal
`view_submission` and `view_closed` lifecycle events before ClawWorks compacts
...
1. ClawWorks downloads the file from Slack's private URL using the bot token.
...
| Forwarded/shared attachments           | Text and Slack-hosted image/file media are best-effort                       | Re-share directly in the ClawWorks thread                                  |
