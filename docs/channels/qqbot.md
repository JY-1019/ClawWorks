...
read_when:
  - You want to connect ClawWorks to QQ
  - You need QQ Bot credential setup
  - You want QQ Bot group or private chat support
title: QQ bot
---

QQ Bot connects to ClawWorks via the official QQ Bot API (WebSocket gateway). The
...
Run multiple QQ bots under a single ClawWorks instance:
...
| `/bot-version` | Show the ClawWorks framework version                                                                     |
...
- Credentials can be backed up and restored as part of standard ClawWorks credential snapshots; the engine re-attaches each account's resource stack on restore without requiring a fresh QR-code pair.

## QR-code onboarding

As an alternative to pasting `AppID:AppSecret` manually, the engine supports a QR-code onboarding flow for linking a QQ Bot to ClawWorks:
...
3. Approve the pairing on the phone. ClawWorks persists the returned credentials into `credentials/` under the right account scope.

Approval prompts generated by the bot itself (for example, "allow this action?" flows exposed by the QQ Bot API) surface as native ClawWorks prompts that you can accept with `/bot-approve` rather than replying through the raw QQ client.
...
- **Repeated self-replies:** ClawWorks records QQ outbound ref indexes as
