...
read_when:
  - You want to connect ClawWorks to WeChat or Weixin
  - You are installing or troubleshooting the openclaw-weixin channel plugin
  - You need to understand how external channel plugins run beside the Gateway
title: "WeChat"
---

ClawWorks connects to WeChat through Tencent's external
...
- `openclaw-weixin` is the ClawWorks channel id.
...
The WeChat code does not live in the ClawWorks core repo. ClawWorks provides the
...
5. The plugin stores account credentials under the ClawWorks state directory.
...
7. Inbound WeChat messages are normalized through the channel contract, routed to
   the selected ClawWorks agent, and sent back through the plugin outbound path.

That separation matters: ClawWorks core should stay channel-agnostic. WeChat login,
...
Direct messages use the normal ClawWorks pairing and allowlist model for channel
plugins.
...
The plugin checks the host ClawWorks version at startup.

| Plugin line | ClawWorks version       | npm tag  |
| ----------- | ----------------------- | -------- |
| `2.x`       | `>=2026.3.22`           | `latest` |
| `1.x`       | `>=2026.1.0 <2026.3.22` | `legacy` |

If the plugin reports that your ClawWorks version is too old, either update
ClawWorks or install the legacy plugin line:
...
Tencent iLink API. In issue #68451, that helper path exposed a bug in ClawWorks's
...
Current ClawWorks startup cleanup excludes the current process and its ancestors,
...
If the Gateway restarts repeatedly after enabling WeChat, update both ClawWorks and
the plugin:
...
JavaScript runtime files ClawWorks needs. Update/reinstall after the plugin
