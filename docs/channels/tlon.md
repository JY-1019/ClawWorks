...
Tlon is a decentralized messenger built on Urbit. ClawWorks connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current ClawWorks releases, so normal packaged
...
By default, ClawWorks blocks private/internal hostnames and IP ranges for SSRF protection.
...
- Thread replies: if the inbound message is in a thread, ClawWorks replies in-thread.
