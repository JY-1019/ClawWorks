---
summary: "Synology Chat webhook setup and ClawWorks config"
read_when:
  - Setting up Synology Chat with ClawWorks
...
Synology Chat ships as a bundled plugin in current ClawWorks releases, so normal
...
3. Point the outgoing webhook URL to your ClawWorks gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in ClawWorks.
...
- ClawWorks accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
```bash
openclaw message send --channel synology-chat --target 123456 --text "Hello from ClawWorks"
openclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
openclaw message send --channel synology-chat --target synology:123456 --text "Short prefix"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before ClawWorks forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. ClawWorks now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached ClawWorks
- `Rate limit exceeded`:
