...
After a successful startup, ClawWorks caches the bot identity in the state directory for up to 24 hours so restarts can avoid an extra Telegram `getMe` call; changing or removing the token clears that cache.
</Note>
...
In Telegram groups and forum topics, an explicit mention of the configured bot handle (for example `@my_bot`) is treated as addressing the selected ClawWorks agent, even when the agent persona name differs from the Telegram username. The group silence policy still applies to unrelated group traffic, but the bot handle itself is not considered "someone else."
...
- DM messages can carry `message_thread_id`; ClawWorks preserves it for replies. DM topic sessions split only when Telegram `getMe` reports `has_topics_enabled: true` for the bot; otherwise DMs stay on the flat session.
...
- Long polling is guarded inside each gateway process so only one active poller can use a bot token at a time. If you still see `getUpdates` 409 conflicts, another ClawWorks gateway, script, or external poller is likely using the same token.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    ClawWorks can stream partial replies in real time:
    ...
    Tool-progress preview updates are the short status lines shown while tools run, for example command execution, file reads, planning updates, patch summaries, or Codex preamble/commentary text in Codex app-server mode. Telegram keeps these enabled by default to match released ClawWorks behavior from `v2026.4.22` and later.
    ...
    <Note>
      Telegram selected quote replies are the exception. When `replyToMode` is `"first"`, `"all"`, or `"batched"` and the inbound message includes selected quote text, ClawWorks sends the final answer through Telegram's native quote-reply path instead of editing the answer preview, so `streaming.preview.toolProgress` cannot show the short status lines for that turn. Current-message replies without selected quote text still keep preview streaming. Set `replyToMode: "off"` when tool-progress visibility matters more than native quote replies, or set `streaming.preview.toolProgress: false` to acknowledge the trade-off.
    </Note>

    For text-only replies:

    - short DM/group/topic previews: ClawWorks keeps the same preview message and performs the final edit in place
    ...
    - if the final edit fails before the completed text is confirmed, ClawWorks uses normal final delivery and cleans up the stale preview

    For complex replies (for example media payloads), ClawWorks falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, ClawWorks skips the preview stream to avoid double-streaming.
...
```
    ...
    - Markdown text is rendered through ClawWorks's Markdown IR and sent as Telegram rich HTML.
...
```
    ...
    - `getMe returned 401` means Telegram rejected the configured bot token. Update `botToken`, `tokenFile`, or `TELEGRAM_BOT_TOKEN` with the current BotFather token; ClawWorks stops before polling so this is not reported as a webhook cleanup failure.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly. ClawWorks pins the spawn confirmation in-topic. Requires `channels.telegram.threadBindings.spawnSessions` to remain enabled (default: `true`).
...
```
    ...
    Sticker descriptions are cached in ClawWorks SQLite plugin state to reduce repeated vision calls.
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, ClawWorks enqueues system events like:
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while ClawWorks is processing an inbound message. `ackReactionScope` decides *when* that emoji is actually sent.
...
```
  ...
  <Accordion title="Long polling vs webhook">
    ...
    In long-polling mode ClawWorks persists its restart watermark only after an update dispatches successfully. If a handler fails, that update remains retryable in the same process and is not written as completed for restart dedupe.
    ...
    ClawWorks then processes the update asynchronously through the same per-chat/per-topic bot lanes used by long polling, so slow agent turns do not hold Telegram's delivery ACK.
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat; only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, ClawWorks preserves the topic for the approval prompt and the follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, ClawWorks now retries these as recoverable network errors.
    - During polling startup, ClawWorks reuses the successful startup `getMe` probe for grammY so the runner does not need a second `getMe` before the first `getUpdates`.
    - If `deleteWebhook` fails with a transient network error during polling startup, ClawWorks continues into long polling instead of making another pre-poll control-plane call. A still-active webhook surfaces as a `getUpdates` conflict; ClawWorks then rebuilds the Telegram transport and retries webhook cleanup.
    ...
    - If logs include `Polling stall detected`, ClawWorks restarts polling and rebuilds the Telegram transport after 120 seconds without completed long-poll liveness by default.
    ...
    - If the ClawWorks managed proxy is configured through `OPENCLAW_PROXY_URL` for a service environment and no standard proxy env is present, Telegram uses that URL for Bot API transport too.
...
<Note>
Multi-account precedence: when two or more account IDs are configured, set `channels.telegram.defaultAccount` (or include `channels.telegram.accounts.default`) to make default routing explicit. Otherwise ClawWorks falls back to the first normalized account ID and `openclaw doctor` warns. Named accounts inherit `channels.telegram.allowFrom` / `groupAllowFrom`, but not `accounts.default.*` values.
</Note>
