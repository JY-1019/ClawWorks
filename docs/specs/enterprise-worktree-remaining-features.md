...
| Palantir AIP-style ontology                         | Foundation implemented, parity not reached | Typed entities, properties, relationships, actions, functions, runtime objects, links, SQLite persistence, and scoped ontology tools exist. Actions write locally, relate two objects over a declared relationship, or call the system that owns the data, and the gate holds an outward call to the action's declared parameters before it leaves. A run holding both the object store and a knowledge corpus is now told, once, which one answers a record question. | The GUI now edits actions (with their local, graph, and outward effects, and their parameters) and derived functions as well as entities, properties, and relationships. A work-map declares no instances: every row is one a run created, so a type its steps update needs a step that creates it. Data-source mapping, dynamic object/property security, richer action logic, object-set queries, aggregations, and application-grade SDKs are absent. |
