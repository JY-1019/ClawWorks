...
ClawWorks has three Vitest suites (unit/integration, e2e, live) and a small set
...
- Codex on-demand install smoke: `pnpm test:docker:codex-on-demand`
  - Installs the packaged ClawWorks tarball in Docker, runs OpenAI API-key
...
- Crestodian first-run Docker smoke: `pnpm test:docker:crestodian-first-run`
  - Starts from an empty ClawWorks state dir, verifies the modern onboard
...
`media`, `e2ee-smoke`, `e2ee-deep`, and `e2ee-cli` jobs. `ClawWorks Release
...
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. It verifies hidden ClawWorks runtime context is persisted as a
...
- `pnpm test:docker:npm-telegram-live`
  - Installs a ClawWorks package candidate in Docker, runs installed-package
...
- `pnpm test:docker:plugins`
  - Packs and installs the current ClawWorks build in Docker, starts the Gateway
...
- `pnpm openclaw qa mantis telegram-desktop-builder`
  - Leases or reuses a Crabbox Linux desktop, installs native Telegram Desktop, configures ClawWorks with a leased Telegram SUT bot token, starts the gateway, and records screenshot/MP4 evidence from the visible VNC desktop.
  ...
  - Telegram Desktop still needs a user login/profile. The bot token configures ClawWorks only. Use `--telegram-profile-archive-env <name>` for a base64 `.tgz` profile archive, or use `--keep-lease` and log in manually through VNC once.
...
- Compare native opus performance in the `libopus-wasm` benchmark repo, not in default ClawWorks install/test loops. Do not set `@discordjs/opus` to `true` in the default `allowBuilds`; that makes unrelated install/test loops compile native code.
...
- Scope:
  ...
  - Exercises ClawWorks's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Docker/Bash E2E lanes that install the packed ClawWorks tarball through `scripts/lib/openclaw-e2e-instance.sh` cap `npm install` at `OPENCLAW_E2E_NPM_INSTALL_TIMEOUT` (default `600s`; set `0` to disable the wrapper for debugging).
...
- ClawWorks bundle MCP tools (real stdio MCP server + embedded ClawWorks profile allow/deny smoke): `pnpm test:docker:agent-bundle-mcp-tools` (script: `scripts/e2e/agent-bundle-mcp-tools-docker.sh`)
...
- Plugin lifecycle matrix smoke: `pnpm test:docker:plugin-lifecycle-matrix` installs the packed ClawWorks tarball in a bare container, installs an npm plugin, toggles enable/disable, upgrades and downgrades it through a local npm registry, deletes the installed code, then verifies uninstall still removes stale state while logging RSS/CPU metrics for each lifecycle phase.
...
ClawWorks gateway container with the OpenAI-compatible HTTP endpoints enabled,
...
inside the container, materializes that server through the embedded ClawWorks bundle
