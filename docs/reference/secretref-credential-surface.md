...
- In scope: strictly user-supplied credentials that ClawWorks does not mint or rotate.
...
- Marker persistence is source-authoritative: ClawWorks writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
- For web search:
