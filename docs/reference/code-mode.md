---
summary: "ClawWorks code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable ClawWorks code mode for an agent run
...
Code mode is an experimental ClawWorks agent-runtime feature. It is off by
default. When you enable it, ClawWorks changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents ClawWorks code mode. It is not Codex Code mode. The two
...
- ClawWorks code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the ClawWorks generic agent runtime, where the model
...
surfaces. ClawWorks code mode is a ClawWorks-owned experimental tool-surface
adapter for generic ClawWorks runs. It uses `quickjs-wasi`, a hidden ClawWorks
tool catalog, and the normal ClawWorks tool executor.

## What is this?

ClawWorks code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal ClawWorks tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same ClawWorks execution path used by normal agent turns.
...
surface. It does not replace ClawWorks tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for ClawWorks, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through ClawWorks
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, ClawWorks fails closed instead of falling back to broad direct tool
  exposure.
...
- Stability: experimental ClawWorks surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic ClawWorks agent runs.
...
**Code mode** is the ClawWorks runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, ClawWorks fails closed for
...
2. Build the effective ClawWorks tool list.
...
- If `language` is `"typescript"`, ClawWorks transpiles before evaluation.
...
final value is JSON-compatible after ClawWorks's output adapter runs.
...
`wait` exists because nested ClawWorks tools can be slow, interactive, approval
...
2. On suspension, ClawWorks snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. ClawWorks re-registers host callbacks by stable names.
5. ClawWorks delivers nested tool results into the restored VM.
6. ClawWorks drains QuickJS pending jobs.
...
const hits = await tools.web_search({ query: "ClawWorks code mode" });
```
...
state directory. For each code-mode `exec` call, ClawWorks builds the run-scoped
...
inside the QuickJS program while ClawWorks still shows only `exec` and `wait` to
the model.
...
7. ClawWorks auto-drains ready namespace bridge calls inside the active
...
catalog for arbitrary enabled ClawWorks, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary ClawWorks tools, not through the
  code-mode bridge
...
Code mode supersedes the ClawWorks Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- ClawWorks does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same ClawWorks executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the ClawWorks compact
catalog bridge. Code mode is the generic ClawWorks alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal ClawWorks
...
If two tools normalize to the same safe convenience name, ClawWorks omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters ClawWorks.
...
ClawWorks loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside ClawWorks's main event loop in a worker. A guest
...
inputs beyond existing ClawWorks trajectory policy.
...
- `tools.search`, `tools.describe`, and `tools.call` work for ClawWorks tools
...
5. Send an agent turn with ClawWorks, plugin, MCP, and client test tools.
...
8. In `exec`, call ClawWorks/plugin/client tools through `tools.search`,
