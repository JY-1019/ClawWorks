---
summary: "Install ClawWorks declaratively with Nix"
read_when:
...
Install ClawWorks declaratively with **[nix-openclaw](https://github.com/openclaw/nix-openclaw)** - the first-party, batteries-included Home Manager module.
...
When `OPENCLAW_NIX_MODE=1` is set (automatic with nix-openclaw), ClawWorks enters a deterministic mode for Nix-managed installs. Other Nix packages can set the same mode; nix-openclaw is the first-party reference.
...
ClawWorks reads JSON5 config from `OPENCLAW_CONFIG_PATH` and stores mutable data in `OPENCLAW_STATE_DIR`. When running under Nix, set these explicitly to Nix-managed locations so runtime state and config stay out of the immutable store.
