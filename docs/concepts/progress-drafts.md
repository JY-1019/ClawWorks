...
read_when:
  ...
  - Explaining how ClawWorks updates one channel message while work is in progress
...
When progress drafts are enabled, ClawWorks creates one visible work-in-progress
...
That is usually enough. ClawWorks will pick an automatic one-word label, wait
...
ClawWorks sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from ClawWorks's built-in
...
ClawWorks renders only the `progress.text` in the channel progress UI. The
...
ClawWorks uses the same formatter for progress drafts and `/verbose`:
...
ClawWorks truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, ClawWorks still suppresses the older standalone
...
When the final answer is ready, ClawWorks tries to keep the chat clean:

- If the draft can safely become the final answer, ClawWorks edits it in place.
- If the channel uses native progress streaming, ClawWorks finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, ClawWorks sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, ClawWorks keeps the
