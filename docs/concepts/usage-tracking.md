...
- `/usage cost` in chats: local cost summary aggregated from ClawWorks session logs.
...
- **MiniMax**: API key or MiniMax OAuth auth profile. ClawWorks treats
  ...
  quota, so ClawWorks inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, ClawWorks prefers the
...
- **DeepSeek**: API key via env/config/auth store (`DEEPSEEK_API_KEY`).
  ClawWorks calls DeepSeek's balance endpoint and shows the provider-reported
...
can supply plugin-specific usage auth logic; otherwise ClawWorks falls back to
