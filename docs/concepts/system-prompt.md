---
summary: "What the ClawWorks system prompt contains and how it is assembled"
read_when:
...
ClawWorks builds a custom system prompt for every agent run. The prompt is **ClawWorks-owned** and does not use a runtime default prompt.

The prompt is assembled by ClawWorks and injected into each agent run.
...
the full ClawWorks-owned prompt. The provider runtime can:
...
- **ClawWorks Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **ClawWorks Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to ClawWorks docs/source and when to read them.
...
ClawWorks keeps large stable content, including **Project Context**, above the
...
ClawWorks can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **ClawWorks
...
For channel auto-reply runs, ClawWorks omits the generic **Silent Replies**
...
ClawWorks keeps committed prompt snapshots for the Codex runtime happy path under
...
ClawWorks developer instructions, turn-scoped collaboration-mode instructions
when ClawWorks provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after ClawWorks sends
...
On the native Codex harness, ClawWorks avoids repeating stable workspace files
...
Codex developer instructions. The compact ClawWorks skills list is also forwarded
...
ClawWorks prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, ClawWorks can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, ClawWorks injects a compact **available skills list**
...
points to the local ClawWorks docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.openclaw.ai](https://docs.openclaw.ai).

The same section also includes the ClawWorks source location. Git checkouts expose the local
...
authority for ClawWorks self-knowledge before the model understands how ClawWorks works,
...
notes, and `memory_search` as instruction context or user memory rather than ClawWorks
