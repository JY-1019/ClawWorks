---
summary: "Move one ClawWorks session's reply route between linked chat channels"
...
Channel docking is call forwarding for one ClawWorks session.
...
Alice can message ClawWorks on Telegram and Discord:
...
ClawWorks keeps the current session context and changes the reply route:
...
4. Keep the same ClawWorks session, but receive future replies in Discord.
