...
- **Control UI**: the sidebar's **Enterprise** group holds one route per surface —
  **Worktree** (`/enterprise`), **History** (`/enterprise/history`), **Tools**
  ...
  - Continuing an interrupted run happens there too. An execution that ended part
    ...
    clicked. `/new` and `/reset` end it: the marker is bound to the transcript
    ...
    One turn can still take it early: an exec-approval followup is generated by
    the gateway but arrives tagged like an operator turn, so if one routes to this
    work-map before your next request, it consumes the marker. Nothing is skipped
    that was not already performed — the completed steps are the named execution's
    own — but the resume is spent on that turn. Arm it again if that happens.
