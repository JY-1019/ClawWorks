---
summary: "Delegate architecture: running ClawWorks as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run ClawWorks as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is a ClawWorks agent that:
...
ClawWorks's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of ClawWorks's own tool policy.
...
- Cron run history: ClawWorks shared SQLite state database
- Session transcripts: `~/.openclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through ClawWorks's session store. For compliance, ensure these logs are retained and reviewed.
...
recall view. ClawWorks redacts credential/token-like text, truncates long
