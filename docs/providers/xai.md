---
summary: "Use xAI Grok models in ClawWorks"
read_when:
  - You want to use Grok models in ClawWorks
  - You are configuring xAI auth or model ids
title: "xAI"
---

ClawWorks ships a bundled `xai` provider plugin for Grok models. For most
users, the recommended path is Grok OAuth with an eligible SuperGrok or X Premium
subscription. ClawWorks stays local-first: the Gateway, config, routing, and
...
app. xAI may still show Grok Build on the consent screen because ClawWorks uses
xAI's shared OAuth client.

## Choose your setup path

Use the path that matches your ClawWorks install state:

<Steps>
  <Step title="New ClawWorks install">
    ...
    On a VPS or over SSH, select xAI OAuth directly; ClawWorks uses device-code
    ...
    OAuth does not require an xAI API key. ClawWorks does not require the Grok
    Build app. xAI may still label the consent app as Grok Build because
    ClawWorks uses xAI's shared OAuth client.

  </Step>
  <Step title="Existing ClawWorks install">
    If ClawWorks is already configured, sign in to xAI only. Do not rerun full
...
<Note>
ClawWorks uses the xAI Responses API as the bundled xAI transport. The same
...
<Tip>
Use `xai-oauth` when signing in from SSH, Docker, or a VPS. ClawWorks prints an
...
ClawWorks includes the current xAI chat models out of the box, ordered newest
...
normalize to `grok-build-0.1`; ClawWorks no longer shows the other retired
...
The bundled plugin maps xAI's current public API surface onto ClawWorks's shared
...
| xAI capability             | ClawWorks surface                         | Status                                                               |
| -------------------------- | ----------------------------------------- | -------------------------------------------------------------------- |
...
| Streaming TTS              | -                                         | Not exposed; ClawWorks's TTS contract returns complete audio buffers |
| Batch speech-to-text       | `tools.media.audio` / media understanding | Yes                                                                  |
| Streaming speech-to-text   | Voice Call `streaming.provider: "xai"`    | Yes                                                                  |
| Realtime voice             | -                                         | Not exposed yet; different session/WebSocket contract                |
| Files / batches            | Generic model API compatibility only      | Not a first-class ClawWorks tool                                     |

<Note>
ClawWorks uses xAI's REST image/video/TTS/STT APIs for media generation,
...
code-execution tools. Features that need different ClawWorks contracts, such as
...
<AccordionGroup>
  ...
  <Accordion title="Video generation">
    ...
    ClawWorks can encode those as data URLs for xAI.
    </Warning>
  ...
  <Accordion title="Image generation">
    ...
    ClawWorks asks xAI for `b64_json` image responses so generated media can be
    ...
    such as `1:2`, `2:1`, `9:20`, and `20:9`. ClawWorks forwards only the
  ...
  <Accordion title="Text-to-speech">
    ...
    <Note>
    ClawWorks uses xAI's batch `/v1/tts` endpoint. xAI also offers streaming TTS
    over WebSocket, but the ClawWorks speech provider contract currently expects
  ...
  <Accordion title="Speech-to-text">
    The bundled `xai` plugin registers batch speech-to-text through ClawWorks's
    ...
    transcription request. Prompt hints are accepted by the shared ClawWorks
  ...
  <Accordion title="x_search configuration">
    The bundled xAI plugin exposes `x_search` as a ClawWorks tool for searching
  ...
  <Accordion title="Code execution configuration">
    The bundled xAI plugin exposes `code_execution` as a ClawWorks tool for
  ...
  <Accordion title="Known limits">
    - xAI auth can use an API key, environment variable, plugin config fallback,
      ...
      API tokens, and the consent page may show Grok Build even though ClawWorks
      does not require the Grok Build app.
    - ClawWorks does not currently expose the xAI multi-agent model family. xAI
      serves these models through the Responses API, but they do not accept the
      client-side or custom tools used by ClawWorks's shared agent loop. See the
      [xAI multi-agent limitations](https://docs.x.ai/developers/model-capabilities/text/multi-agent#limitations).
    - xAI Realtime voice is not registered as a ClawWorks provider yet. It
  ...
  <Accordion title="Advanced notes">
    - ClawWorks applies xAI-specific tool-schema and tool-call compatibility fixes
    ...
    - `web_search`, `x_search`, and `code_execution` are exposed as ClawWorks
      tools. ClawWorks enables the specific xAI built-in it needs inside each tool
...
shared image live file verifies the same xAI provider through ClawWorks's
