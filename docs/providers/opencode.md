---
summary: "Use OpenCode Zen and Go catalogs with ClawWorks"
read_when:
...
OpenCode exposes two hosted catalogs in ClawWorks:
...
Both catalogs use the same OpenCode API key. ClawWorks keeps the runtime provider ids
...
<AccordionGroup>
  ...
  <Accordion title="Gemini replay behavior">
    Gemini-backed OpenCode refs stay on the proxy-Gemini path, so ClawWorks keeps
