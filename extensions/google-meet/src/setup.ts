...
export function getGoogleMeetSetupStatus(
  ...
  checks.push({
    ...
    message: config.chrome.browserProfile
      ? "Local Chrome uses the ClawWorks browser profile; chrome.browserProfile is passed to chrome-node hosts"
      : "Local Chrome uses the ClawWorks browser profile; configure browser.defaultProfile to choose another profile",
  });
