/**
 * Blocks direct Codex app-server requests that would bypass ClawWorks sandbox or
...
/** Returns a block message when a direct app-server method would bypass ClawWorks execution policy. */
...
function formatCodexNativeSandboxBlock(params: { surface: string }): string {
  return [
    `Codex-native ${params.surface} is unavailable because ClawWorks sandboxing is active for this session.`,
    "This mode cannot route execution through the ClawWorks sandbox backend.",
