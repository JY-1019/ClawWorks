...
describe("codex conversation binding", () => {
  ...
  it("rejects binding when configured exec auto mode may need unrouted human approvals", async () => {
    ...
    await expect(
      ...
      "ClawWorks native Codex conversation binding cannot route interactive approvals yet",
    );
  ...
  it("blocks bound Codex app-server turns when the current ClawWorks session is sandboxed", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    await fs.writeFile(
      `${sessionFile}.codex-app-server.json`,
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because ClawWorks sandboxing is active for this session.",
        ),
      },
    });
  ...
  it("blocks bound Codex app-server turns when exec host=node is active", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex app-server conversation binding is unavailable because ClawWorks exec host=node is active for this session.",
        ),
      },
    });
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  ...
  it("blocks bound Codex app-server turns when the binding agent uses node exec without a session key", async () => {
    ...
    expect(result?.reply?.text).toContain("ClawWorks exec host=node is active");
    expect(sharedClientMocks.getSharedCodexAppServerClient).not.toHaveBeenCalled();
  });
  ...
  it("blocks bound Codex CLI node turns when the current ClawWorks session is sandboxed", async () => {
    ...
    expect(result).toEqual({
      ...
      reply: {
        text: expect.stringContaining(
          "Codex-native Codex CLI node conversation binding is unavailable because ClawWorks sandboxing is active for this session.",
        ),
      },
    });
